...
class App:
    ...
    def create_content_and_match_with_info(self, url, title, date, n_view, n_comment, n_reactions, good, bad, hashtags, description, crawl_time, category, rank, creator, creator_url, platform):
        ...
        with self.driver.session() as session:
            ...
            if len(result) == 0:
                result = session.write_transaction(
                    self._create_content_and_match_with_info, url, title, date, n_view, n_comment, n_reactions, good, bad, hashtags, description, crawl_time, category, rank, creator, creator_url, platform
                )
            else:
                result = session.write_transaction(
    ...
    def _find_and_return_creator(tx, creator, creator_url):
        ...
        query = (
            "MATCH (c:Creator { name: $creator, type: 'person', creator_url: $creator_url }) "
    ...
    def _create_creator(tx, creator, creator_url):
        '''create_creator_and_match_platform method'''
        query = (
            "CREATE (c: Creator { name: $creator, type: 'person', creator_url: $creator_url }) "
            "return c"
        )
        tx.run(query, creator=creator, creator_url=creator_url)
    ...
    def _find_edge_with_creator_and_platform(tx, creator, creator_url, platform):
        ...
        query = (
            ...
            "WHERE c.name = $creator AND c.type = 'person' AND c.creator_url = $creator_url AND p.name = $platform "
            "RETURN c.name AS name "
        )
    ...
    @staticmethod
    def _create_content_and_match_with_info(tx, url, title, date, n_view, n_comment, n_reactions, good, bad, hashtags, description, crawl_time, category, rank, creator, creator_url, platform):
        '''create_content_and_match_with_info method'''
        query = (
            "CREATE (c:Content:" + crawl_time + " { url: $url, title: $title, date: $date, n_view: $n_view, n_comment: $n_comment, n_reactions: $n_reactions, good: $good, bad: $bad, hashtags: $hashtags, description: $description }) "
            "WITH c "
            "MATCH (t:Type { name: $category }) "
            "MATCH (r:Rank { n: $rank, platform: $platform }) "
            "MATCH (cr:Creator) WHERE cr.name = $creator AND cr.type = 'person' AND cr.creator_url = $creator_url "
            "MERGE (c)<-[:INCLUDES]-(t) "
            "MERGE (c)<-[:RANKS]-(r) "
            "MERGE (c)<-[:MAKES]-(cr) "
        )
        tx.run(query, url=url, title=title, date=date, n_view=n_view, n_comment=n_comment,\
            n_reactions=n_reactions, good=good, bad=bad, hashtags=hashtags, description=description, category=category,\
            rank=rank, creator=creator, creator_url=creator_url, platform=platform)
...
if __name__ == "__main__":
    ...
    file_list_py = [file for file in file_list if file.startswith('pre_youtube_data')]
