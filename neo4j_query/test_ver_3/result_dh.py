...
class App:
    ...
    def insert_edge_weight_fi(self, crawl_label):
        # feature importance 정보를 가지고 edge에 weight를 넣어줌
        with self.driver.session() as session:
            session.write_transaction(self._insert_edge_weight_naver, crawl_label)
            session.write_transaction(self._insert_edge_weight_daum, crawl_label)
            session.write_transaction(self._insert_edge_weight_youtube, crawl_label)
    ...
    def ranked_by_feature_importance(self, crawl_time):
        with self.driver.session() as session:
            ...
            naver_keys = ['id','n_reactions','good','warm','sad','angry','want','n_comment', 'pagerank','betweeness','closeness','n_view','degree']
            ...
            daum_keys = ['id', 'n_reactions','like','impress','angry','sad','recommend', 'n_comment','pagerank','betweeness','closeness','degree']
            ...
            print(daum_df.head())
            print(daum_df.index[0])
            print(daum_df.iloc[0, -1])

            
            youtube_all = session.read_transaction(self._get_content_youtube, crawl_time)
            youtube_keys = ['id', 'n_reactions','good','bad','n_comment','n_view','pagerank','betweeness','closeness','degree']
            ...
            print(youtube_df.index[0])
            print(youtube_df.iloc[0, -1])
        ...
        with self.driver.session() as session:
            for key in keywords:
                ...
                for k in nkey:
                    ...
                    k_tmp['weight'] = k[2]
                    k_list.append(k_tmp)
                ncontent = session.read_transaction(self._neighbor_content, id, crawl_label)
                nc_list = []
                for nc in ncontent:
                    nc_tmp = dict()
                    nc_tmp['id'] = nc[0]
    ...
    @staticmethod
    def _neighbor_content(tx, id, crawl_label):
        query = (
            "MATCH (n:Keyword) WHERE ID(n) = $id "
            "WITH n "
            "MATCH (n:Keyword)<-[:EXTRACTS]-(m:"+crawl_label+") "
            "RETURN ID(m) AS id, m.title AS title, m.url AS url, e.weights_sum as es "
            "ORDER BY es DESC "
            "LIMIT 7"
        )
        res = tx.run(query, id=id)
        res = [[i['id'],i['title'], i['url'], i['weight']] for i in res]
        return res
    ...
    def _get_content_naver(tx, crawl_time):
        query = (
            ...
            "RETURN ID(c) AS id, c.n_reactions AS n_reactions, c.good AS good, c.warm AS warm, c.sad AS sad, c.angry AS angry, c.want AS want, "
            "c.n_comment AS n_comment, c.pagerank AS pagerank, c.betweeness AS betweeness, c.closeness AS closeness, "
            "c.n_view AS n_view, c.degree AS degree"
        )
        result = tx.run(query)
        return [[row['id'], row['n_reactions'], row['good'], row['warm'], row['sad'], row['angry'], row['want'], 
                row['n_comment'], row['pagerank'],  row['betweeness'], row['closeness'], row['n_view'], row['degree']]  for row in result]

    @staticmethod
    def _get_content_daum(tx, crawl_time):
        query = (
            "MATCH (c:Content:"+crawl_time+")<-[:INCLUDES]-(t:Type)<-[:CLASSIFIES]-(p:Platform { name: 'Daum' })"
            "RETURN ID(c) AS id, c.n_reactions AS n_reactions, c.like AS like, c.impress AS impress, c.angry AS angry, c.sad AS sad, c.recommend AS recommend, "
            "c.n_comment AS n_comment, c.pagerank AS pagerank, c.betweeness AS betweeness, c.closeness AS closeness, c.degree AS degree"
        )
        result = tx.run(query)
        return [[row['id'], row['n_reactions'], row['like'], row['impress'], row['angry'], row['sad'], row['recommend'], 
                row['n_comment'], row['pagerank'], row['betweeness'], row['closeness'], row['degree']] for row in result]

    @staticmethod
    def _get_content_youtube(tx, crawl_time):
        query = (
            "MATCH (c:Content:"+crawl_time+")<-[:INCLUDES]-(t:Type)<-[:CLASSIFIES]-(p:Platform { name: 'Daum' })"
            "RETURN ID(c) AS id, c.n_reactions AS n_reactions, c.good AS good, c.bad AS bad, c.n_comment AS n_comment, "
            "c.n_view AS n_view, c.pagerank AS pagerank, c.betweeness AS betweeness, c.closeness AS closeness, c.degree AS degree"
        )
        result = tx.run(query)
        return [[row['id'], row['n_reactions'], row['good'], row['bad'], row['n_comment'], row['n_view'], 
                row['pagerank'], row['betweeness'], row['closeness'], row['degree']] for row in result]

    @staticmethod
    def _get_importance_naver(tx):
        query = (
            "MATCH (c : Importance { platform: 'Naver' }) "
            "RETURN c.n_reactions AS n_reactions, c.good AS good, c.warm AS warm, c.sad AS sad, c.angry AS angry, c.want AS want, "
            "c.n_comment AS n_comment, c.pagerank AS pagerank, c.betweeness AS betweeness, c.closeness AS closeness, "
            "c.n_view AS n_view, c.degree AS degree"
        )
        result = tx.run(query)
        return [[row['n_reactions'], row['good'], row['warm'], row['sad'], row['angry'], row['want'], 
                row['n_comment'], row['pagerank'],  row['betweeness'], row['closeness'], row['n_view'], row['degree']]  for row in result]
    ...
    def _get_importance_daum(tx):
        query = (
            ...
            "RETURN c.n_reactions AS n_reactions, c.like AS like, c.impress AS impress, c.angry AS angry, c.sad AS sad, c.recommend AS recommend, "
            "c.n_comment AS n_comment, c.pagerank AS pagerank, c.betweeness AS betweeness, c.closeness AS closeness, c.degree AS degree"
        )
        result = tx.run(query)
        return [[row['n_reactions'], row['like'], row['impress'], row['angry'], row['sad'], row['recommend'], 
                row['n_comment'], row['pagerank'], row['betweeness'], row['closeness'], row['degree']] for row in result]
    ...
    def _get_importance_youtube(tx):
        query = (
            ...
            "RETURN c.n_reactions AS n_reactions, c.good AS good, c.bad AS bad, c.n_comment AS n_comment, "
            "c.n_view AS n_view, c.pagerank AS pagerank, c.betweeness AS betweeness, c.closeness AS closeness, c.degree AS degree"
        )
        result = tx.run(query)
        return [[row['n_reactions'], row['good'], row['bad'], row['n_comment'], row['n_view'], 
                row['pagerank'], row['betweeness'], row['closeness'], row['degree']] for row in result]
    ...
    def _ranked_by_weights_sum(tx, crawl_time):
        ...
        query2 = (
            "MATCH (n:Keyword :"+crawl_time+")<-[e:EXTRACTS]-(c:Content:"+crawl_time+") "
            "WITH e, c, n "
            "MATCH (c)<-[:INCLUDES]-(t:Type)<-[:CLASSIFIES]-(p:Platform ) "
            "RETURN e.weights_sum AS weights_sum, p.name AS name "
        )
        result = tx.run(query)
        result2 = tx.run(query2)
        res = [[row["weights_sum"],row['name']] for row in result2]
        print(res)
        res = [[row["name"],row['id'],row['sum_rank']] for row in result]
        return res
