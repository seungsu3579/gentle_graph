...
class App:
    ...
    def insert_edge_weight_pure(self, crawl_label):
        ...
        for i in range(24):
            crawl_time = crawl_label + "{0:0>2}".format(i)
            with self.driver.session() as session:
                session.write_transaction(self._insert_edge_weight_naver, crawl_time)
                session.write_transaction(self._insert_edge_weight_daum, crawl_time)
                session.write_transaction(self._insert_edge_weight_youtube, crawl_time)
    ...
    def ranked_by_pure_rank(self, crawl_label):
        ...
        with self.driver.session() as session:
            ...
            for i in range(24):
                ...
                tmp = session.read_transaction(self._ranked_by_pure_rank, crawl_time)
                tmp_res = []
                idx = 1
                for elem in tmp:
    ...
    def ranked_by_degree(self, crawl_label):
        with self.driver.session() as session:
            for i in range(24):
                crawl_time = crawl_label + "{0:0>2}".format(i)
                tmp = session.read_transaction(self._ranked_by_degree, crawl_time)
                result = []
                tmp_res = []
                idx = 1
                for elem in tmp:
    ...
    def ranked_by_feature_importance(self, crawl_label):
        with self.driver.session() as session:
            for i in range(24):
                crawl_time = crawl_label + "{0:0>2}".format(i)
                scaler = MinMaxScaler()
