function draw_window(){
  ...
  d3.json("keyword_info_2.json", function(json) {
    ...
    node.append("text")
        ...
        .attr("class", "keyword")
        .attr("id", function(d) { return d.id; })
        .attr("font-size", function(d) { return d.weights * 20 + 10;});
...
function draw_detail(node_id){
  ...
  d3.json("keyword_info_2.json", function(json) {
    ...
    function tick(){
      node.attr("transform", function(d) { return "translate(" + (d.type == 'keyword' ? d.x - width * 0.20 :  d.x + width * 0.10 )+ "," + (d.type == 'content' ? d.y + d.cnt * 50 - 200 : d.y + 300 ) + ")"; });
    }
